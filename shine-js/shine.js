...
import createModule from './shine.stubs'
...
Shine.prototype._encodePass = function (data) {
  ...
  const _buf = ShineModule._shine_encode_buffer(this._handle, this._buffer, this._written)

  const written = ShineModule.getValue(this._written, 'i16')
...
function concat(ctr, a, b) {
  ...
  const ret = new ctr(a.length + b.length)
  ret.set(a)
...
function convertFloat32(buf) {
  const ret = new Array(buf.length)
...
Shine.prototype.encode = function (data) {
  ...
  const tmp = new Array(this._channels)
...
Shine.prototype.close = function () {
  ...
  const _buf = ShineModule._shine_flush(this._handle, this._written)

  const written = ShineModule.getValue(this._written, 'i16')
  const encoded = new Uint8Array(written)
...
Shine.initialized = (createModule()).then((Module) => {
  ShineModule = Module
  shineInit()
})
